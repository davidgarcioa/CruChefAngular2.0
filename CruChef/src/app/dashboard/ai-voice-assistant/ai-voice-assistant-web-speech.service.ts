...
export interface VoiceRestaurantContext {
  ownerUid?: string;
  restaurantId: string;
...
export class AiVoiceAssistantService {
  ...
  async sendTextToBackend(
    transcript: string,
    restaurant: VoiceRestaurantContext,
  ): Promise<DishResponse> {
    try {
      console.log('[Voice] Enviando al backend:', { transcript, restaurant });

      const payload = {
        transcript: transcript.trim(),
        restaurant_id: restaurant.restaurantId,
        owner_uid: restaurant.ownerUid,
      ...
      if (error.status === 0) msg = 'Backend no disponible. Verifica el despliegue.';
  ...
  async createDishFromVoice(
    ...
    try {
      console.log('[Voice] 🎙️ Creando plato desde voz:', { name, price, category, restaurant });
  ...
  async sendAudioToBackend(
    ...
    return this.sendTextToBackend(txt, restaurant);
  }
